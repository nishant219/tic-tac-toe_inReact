...
const App=()=> {
  ...
  const reloadGame = () => {
    //to reload game reset values of set and handle item array
    setIsCross(false);
    setWinMessage("");
  ...
  const changeItem = itemNumber => {
    //if won the game show msg else keep flipping cards
    if (winMessage) {
      return toast(winMessage, { type: "success" });
    }
    //if card is empty i.e not flipped then check whether it is 0 or * and change flip for next player
    // else card already flipped  
    ...
    <Container className="p-5">
      ...
      <Row>
        <Col md={6} className="offset-md-3">
          ...
          {/* to display win message  -->  ? ():()  and then provide reload btn if over*/}
          {winMessage ? (
